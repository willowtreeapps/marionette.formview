...
;(function (root, factory) {
  ...
  var FormView = Marionette.FormView = Marionette.ItemView.extend({
    ...
    validateRule : function (val,validationRule) {
      ...
      if (validationRule === 'required') return this.formValidator.required(val);
      ...
      if (this.rules && this.rules[validationRule]) {
        ...
        if (_.isFunction(this.formValidator[validationRule])) {
          return _(this.formValidator[validationRule]).bind(this)(val,options);
        }
      }
    ...
    formValidator : {
        ...
        validate : function(validator, val, options) {
          if (_.isFunction(this[validator])) return _(this[validator]).bind(this)(val,options);
        ...
        alpha : function(val) {
          return this.formValdator.regex.alpha.test(val);
        },

        alphanum : function (val) {
          return this.formValidator.regex.alphanum.test(val);
        },

        email :function(val) {
            return this.formValidator.regex.email.test(val);
        },
